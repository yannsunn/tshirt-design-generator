...
### **0. タスク管理ルール**
⚠️ **最重要**: AIアシスタント（Claude）は忘れやすい性質があるため、必ず以下を実行

**必須事項:**
1. **すべての作業をタスクリストに起こす**
   - ユーザーからの指示を受けたら、即座にTodoWriteツールでタスク化
   - 複数の作業がある場合は、すべて個別のタスクとして記録
   - 作業中も進捗に応じてタスクを更新

2. **タスクの状態を常に最新に保つ**
   - 作業開始時: `in_progress` に変更
   - 作業完了時: `completed` に変更（即座に）
   - 新しい問題発見時: 新規タスクを追加

3. **忘れてはいけない情報の記録**
   - ユーザーの重要な指示は、タスクの `content` に詳細を記載
   - 問題の原因や解決策も記録
   - 次回の作業で参照できるようにする
