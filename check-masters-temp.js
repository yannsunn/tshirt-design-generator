...
const options = {
  ...
  path: '/v1/shops/24565480/products.json?limit=50',
  method: 'GET',
  headers: {
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    ...
    const masters = json.data.filter(p => p.title.includes('MASTER'));
    ...
    masters.forEach(p => {
      ...
      console.log(`Provider: ${p.print_provider_id}`);
      console.log('---');
    });
    console.log(`\n合計: ${masters.length}個のMASTER商品`);
  });
});
